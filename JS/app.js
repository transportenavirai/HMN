...
// Filtro Hoje / Ontem / Todos
let vFiltroAtivo = 'todos';

function filtroViagens(modo) {
  vFiltroAtivo = modo;
  document.querySelectorAll('.vfiltro-btn').forEach(function(b) { b.classList.remove('active'); });
  const mapa = { hoje:'vfHoje', ontem:'vfOntem', todos:'vfTodos' };
  const el = document.getElementById(mapa[modo]);
...
function renderViagensTbody(hojeKey, ontemKey) {
  ...
  let base = viagensCache;
  if (vFiltroAtivo === 'hoje')  base = viagensCache.filter(function(r){ return dtToKey(r.dt) === hojeKey; });
  if (vFiltroAtivo === 'ontem') base = viagensCache.filter(function(r){ return dtToKey(r.dt) === ontemKey; });
  const filtered = q
